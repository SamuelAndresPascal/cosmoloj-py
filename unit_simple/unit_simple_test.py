...
def test_metric_prefix():
    ...
    assert pytest.approx(expected=3., rel=1e-10) == cm_to_km.inverse().convert(0.00003)
...
def test_derived():
    ...
    assert pytest.approx(expected=30000000000., rel=1e-10) == km2_to_cm2.convert(3.)
    assert pytest.approx(expected=3., rel=1e-10) == km2_to_cm2.inverse().convert(30000000000.)
...
def test_combined_dimension_derived():
    ...
    assert pytest.approx(expected=1., rel=1e-10) == g_per_m2_to_ton_per_km2.convert(1.)
    assert pytest.approx(expected=3., rel=1e-10) == g_per_m2_to_ton_per_km2.inverse().convert(3.)
    assert pytest.approx(expected=1e-10, rel=1e-20) == g_per_m2_to_ton_per_cm2.convert(1.)
    assert pytest.approx(expected=3e-10, rel=1e-20) == g_per_m2_to_ton_per_cm2.convert(3.)
    ...
    assert pytest.approx(expected=3., rel=1e-10) == g_per_m2_to_ton_per_cm2.inverse().convert(3e-10)
...
def test_temperatures():
    ...
    assert pytest.approx(expected=-273.15, rel=1e-10) == k_to_c.convert(0)
    assert pytest.approx(expected=273.15, rel=1e-10) == k_to_c.inverse().convert(0)
    ...
    assert pytest.approx(expected=3., rel=1e-10) == k_per_m_to_c_per_m.inverse().convert(3.)
...
def test_speed():
    ...
    assert pytest.approx(expected=360., rel=1e-10) == ms_to_kmh.convert(100.)
    assert pytest.approx(expected=5., rel=1e-10) == ms_to_kmh.inverse().convert(18.)
